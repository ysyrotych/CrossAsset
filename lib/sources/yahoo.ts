...
// v8/finance/chart works without crumb authentication — no session needed
  ...
  for (const host of ["query1", "query2"]) {
    try {
      const r = await fetch(
        ...
        `?interval=1d&range=${range}&events=splits,dividends`,
        {
      ...
      const adjCl: (number|null)[]  = d.indicators?.adjclose?.[0]?.adjclose ?? closes;
...
// Batch-fetch adjusted history for multiple symbols — all in parallel, no auth needed
export async function fetchAdjustedHistoryBatch(
  ...
  const rows = await Promise.all(
    symbols.map(async sym => ({ sym, bars: await fetchAdjustedHistoryOne(sym, range) }))
  );
