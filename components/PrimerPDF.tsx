...
const S = StyleSheet.create({
  ...
  // Section
  sectionHeader: { backgroundColor: NAVY, color: "white", fontSize: 7, fontFamily: "Helvetica-Bold",
    letterSpacing: 1.2, textTransform: "uppercase", paddingHorizontal: 8, paddingVertical: 4, marginBottom: 8, marginTop: 16 },
  subsectionHeader: { fontSize: 9, fontFamily: "Helvetica-Bold", color: NAVY, marginBottom: 5, marginTop: 10, borderBottomWidth: 1, borderBottomColor: BORDER, paddingBottom: 2 },
  ...
  colHeaderBear: { fontSize: 7.5, fontFamily: "Helvetica-Bold", color: "#b42318", letterSpacing: 0.6, textTransform: "uppercase", marginBottom: 6, borderBottomWidth: 1, borderBottomColor: "#fca5a5", paddingBottom: 3 },
...
function parsePrimerSections(markdown: string): Record<string, string> {
  ...
  for (const line of lines) {
    ...
    if (h2) {
      flush();
      currentKey = h2[1].trim().toUpperCase().replace(/\s+/g, "_");
      buf = [];
    } else if (h3) {
      flush();
      currentKey = h3[1].trim().toUpperCase().replace(/\s+/g, "_");
      buf = [];
    } else {
      buf.push(line);
    }
...
export function PrimerDocument({ ticker, companyName, industry, content, generatedDate, history, facts }: PrimerPDFProps) {
  ...
  // Build 5-year financial summary table
    ...
    <Document title={`${companyName} (${ticker}) — Equity Research Primer`} author="Edgewood Management">
      <Page size="LETTER" style={S.page}>
        ...
        <View style={S.body}>
            ...
            <>
              ...
              <View style={S.table}>
                ...
                {[
                  ...
                  { label: "Operating Income", data: history.operating_income },
          ...
          {bg.length > 0 && <>
            <SubSectionHdr title="Company Background" />
            {bg.map((p, i) => <Para key={i} text={p} />)}
          </>}
          {pp.length > 0 && <>
            <SubSectionHdr title="Product Portfolio & Revenue Mix" />
            {pp.map((p, i) => <Para key={i} text={p} />)}
          </>}
          {cust.length > 0 && <>
            <SubSectionHdr title="Customers, End Markets & Geographic Exposure" />
            {cust.map((p, i) => <Para key={i} text={p} />)}
          </>}

          {/* ── Industry Analysis ── */}
          <SectionHdr title="IV. Industry Analysis" />
          {mkt.length > 0 && <>
            <SubSectionHdr title="Market Structure & Competitive Dynamics" />
            {mkt.map((p, i) => <Para key={i} text={p} />)}
          </>}
          {drv.length > 0 && <>
            <SubSectionHdr title="Key Industry Drivers & Cycle" />
            {drv.map((p, i) => <Para key={i} text={p} />)}
          </>}
          {comp.length > 0 && <>
            <SubSectionHdr title="Competitive Position" />
            {comp.map((p, i) => <Para key={i} text={p} />)}
          </>}
          ...
          {/* ── Key Risks ── */}
          {risks.length > 0 && <>
            <SectionHdr title="VI. Key Risks" />
            <Bullets items={risks} color="#b42318" />
          </>}

          {/* ── Investment Thesis ── */}
          <SectionHdr title="VII. Investment Thesis" />
          <View style={S.twoCols}>
            ...
            <View style={S.col}>
              ...
              <Bullets items={bear} color="#b42318" />
            </View>
          </View>
