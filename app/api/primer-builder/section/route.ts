...
const SECTION_PROMPTS: Record<string, string> = {
  executive_summary:
    "Write a high-conviction executive summary AS A BULLET LIST — use '•' bullets, one per idea. This section is the LAST THING WRITTEN — synthesize from the ALREADY WRITTEN sections above, pulling specific numbers and conclusions already established. Required bullets in order: (1) Open with the single most important thing an investor needs to know right now — cite the specific ROIC-WACC spread or FCF yield vs risk-free rate from COMPUTED QUALITY METRICS; (2) What the business does and WHY the model generates above-average returns (or why it doesn't) — name the specific structural advantage — pull this from the Business Overview and Industry Analysis if available in ALREADY WRITTEN; (3) Financial quality argument: FCF margin trajectory, OCF/NI earnings quality ratio, and operating leverage evidence with specific dollar amounts — cite numbers already in the Financial Analysis section if written; (4) Valuation — NTM multiple from ANALYST FORWARD ESTIMATES vs what revenue growth and margin that embeds — if Valuation Framework is written, reference the specific scenario prices already there; (5) Bull case in one sentence with a specific price target and the key catalyst — draw from Investment Thesis bull case if written; (6) Bear case in one sentence with a specific downside number and what triggers it — draw from Investment Thesis bear case if written; (7) Bottom line stance — use EXACTLY this format: '[BUY/HOLD/SELL] — 12-month price target $[number]. [One sentence on conviction and key catalyst.]' No hedging. Every bullet must contain at least one number. CRITICAL: check ALREADY WRITTEN sections for the price targets and valuation math — do not invent numbers that contradict what is already written.",
  ...
  key_metrics:
    "Write a key metrics dashboard for 7-8 KPIs specific to THIS business model. Current value, NTM consensus estimate if available from ANALYST FORWARD ESTIMATES, specific watch threshold that changes the thesis, and why this metric matters more than standard metrics for this company. NEVER include Rule of 40 or Piotroski F-Score — these are screener metrics, not PM-grade analytics. Use business-model-specific KPIs: for marketplaces use take rate / gross bookings growth; for SaaS use NRR / ARR; for financials use NIM / efficiency ratio; for industrials use book-to-bill. Always include: (1) Revenue CAGR trend and NTM consensus, (2) Gross margin trajectory, (3) FCF conversion (OCF/NI), (4) ROIC/WACC spread, (5) CapEx intensity, (6) Net Debt/EBITDA, (7-8) business-model-specific KPIs. Format strictly as a table: KPI | LTM Value | NTM Est | Watch Threshold | Why It Matters Here.",
